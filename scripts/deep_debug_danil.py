...
async def debug_user(tg_id):
    ...
    # Try both DBs just in case
    for db_id in ["(default)", "test-db-123456789"]:
        print(f"Checking DB: {db_id}")
        os.environ["FIREBASE_DATABASE_ID"] = db_id
        # Force re-initialization of firestore client if needed? 
        # Actually FirestoreDB.db is set at module level. I'll use the client directly if possible.
        ...
        if user_data:
            ...
            if start_date:
                ...
                try:
                    ...
                    if task:
                        print(f"Task 2.0 found: {task.get('day_name')}")
                    else:
                        print("Task 2.0 NOT found. Trying fallback...")
                        ...
                        if fallback:
                            ...
                            print("❌ NO TASK FOUND FOR DAY", day)
                except Exception as e:
                    print(f"❌ Error fetching task: {e}")
            else:
                print("❌ NO START DATE found")
                
            print(f"Last Morning: {user_data.get('last_morning_sent')}")
            print(f"Morning Time: {user_data.get('morning_time')}")
            print(f"Timezone: {user_data.get('timezone')}")
            print(f"--- End of User Data ---")

if __name__ == "__main__":
    for uid in [1819873644, 185472014]:
